...
const PORT =
  ...
  isNaN(parseInt(process.env.PORT.trim()))
    ? 5000
...
const CACHE_TIMEOUT_MINS =
  !process.env.CACHE_TIMEOUT_MINS ||
  process.env.CACHE_TIMEOUT_MINS.trim() === "" ||
  isNaN(parseInt(process.env.CACHE_TIMEOUT_MINS.trim()))
    ? 0
    : parseInt(process.env.CACHE_TIMEOUT_MINS.trim());
...
if (
  ...
  process.env.API_KEY_QUERY_PARAM.trim() === ""
) {
...
app.listen(PORT, HOST, () =>
  console.log(`Proxy server running on host ${HOST} and port ${PORT}`),
);
